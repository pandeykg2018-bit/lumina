...
import { ArrowUpRight, ChevronRight, Menu, Moon, Network, Search, Sun, X } from 'lucide-react'
...
function Nav({ theme, onToggleTheme, onOpenLibrary }: { theme: 'light' | 'dark'; onToggleTheme: () => void; onOpenLibrary: () => void }) {
  const [open, setOpen] = useState(false)
  return <>
    <header className="nav-wrap"><a className="brand" href="#top" aria-label="Social Norm home"><span className="brand-mark"><span /></span><span>social norm<small>by Parivestra Ventures</small></span></a><nav className="desktop-nav" aria-label="Main navigation"><button className="agents-nav-button" onClick={onOpenLibrary}>Agents <span>⌄</span></button><a href="#workflows">Workflows</a><a href="#categories">Categories</a><a href="#about">About</a></nav><div className="nav-actions"><a href="#about" className="signin">Sign in</a><button className="theme-toggle" onClick={onToggleTheme} aria-label={`Switch to ${theme === 'light' ? 'dark' : 'light'} theme`} title={`Switch to ${theme === 'light' ? 'dark' : 'light'} theme`}>{theme === 'light' ? <Moon /> : <Sun />}</button><a href="#agents" className="dark-button">Get started <ArrowUpRight size={15} /></a><button className="mobile-menu" onClick={() => setOpen(true)} aria-label="Open menu"><Menu /></button></div></header>
